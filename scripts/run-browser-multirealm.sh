...
set -euo pipefail
output=${1:-artifacts}
duration=${MULTIREALM_DURATION_MS:-2000}
mkdir -p "$output"
...
HOME="$profile" "$chrome" \
  ...
  "http://127.0.0.1:$port/browser-multirealm-example.html?duration=$duration" \
  >"$browser_log" 2>&1 &
browser_pid=$!
cleanup() {
  status=$?
  ...
  if [[ $status -ne 0 ]]; then
    ...
    echo "--- browser log ---" >&2; cat "$browser_log" >&2
  fi
  rm -rf "$profile" "$server_log" "$browser_log" 2>/dev/null || true
  exit "$status"
}
trap cleanup EXIT
node tests/browser/collect-multirealm.mjs "$debug_port" "$output"
